...
interface Duration { start: string; end: string; }
...
interface Education { degree: string; institution: string; duration: Duration; }
interface TechnicalSkills { [key: string]: string[]; }
interface ResumeData {
  id: string; userId: string; profileType: string;
  fullName: string; email: string; phone: string; location: string;
  professionalSummary: string; links: ResumeLinks;
  education: Education; technicalSkills: TechnicalSkills;
...
export default function ResumeEditorPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await api.patch(`/resumes/${data.id}`, {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const res = await api.get(`/resumes/${data.id}/download`, { responseType: 'blob' });
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${data.profileType?.replace(/\s+/g, '_') || 'Resume'}.pdf`;
      a.click();
  ...
  // ── Live preview HTML ───────────────────────
  const previewHtml = data ? generatePreviewHtml(data) : '';
...
function PersonalTab({ data, update }: { data: ResumeData; update: <K extends keyof ResumeData>(k: K, v: ResumeData[K]) => void }) {
  return (
    <div className="space-y-7 pb-4">
      <div className="space-y-1.5">
        <Field label="Profile Name" value={data.profileType} onChange={(v) => update('profileType', v)} placeholder="e.g., Frontend Developer" />
        <p className="text-[11.5px] text-muted-foreground/60">Internal name for this profile (e.g., Frontend Developer)</p>
      </div>
...
function ExperienceTab({ data, update }: { data: ResumeData; update: <K extends keyof ResumeData>(k: K, v: ResumeData[K]) => void }) {
    ...
    <div className="space-y-6">
      {exps.map((exp, idx) => (
        <div key={idx} className="p-4 rounded-xl bg-card/40 border border-border/30 space-y-4">
          ...
          />
          <p className="text-xs text-muted-foreground/60">E.g., React, Node.js, TypeScript</p>
...
function SkillsTab({ data, update }: { data: ResumeData; update: <K extends keyof ResumeData>(k: K, v: ResumeData[K]) => void }) {
    ...
    <div className="space-y-5">
      ...
      {categories.map(([cat, vals]) => (
        <div key={cat} className="p-4 rounded-xl bg-card/40 border border-border/30 space-y-3">
          ...
          <p className="text-xs text-muted-foreground/50">Example: React, Node.js, TypeScript</p>
        </div>
      ))}
    </div>
  );
...
function ProjectsTab({ data, update }: { data: ResumeData; update: <K extends keyof ResumeData>(k: K, v: ResumeData[K]) => void }) {
...
function EducationTab({ data, update }: { data: ResumeData; update: <K extends keyof ResumeData>(k: K, v: ResumeData[K]) => void }) {
...
// ══════════════════════════════════════════════
// PREVIEW HTML GENERATOR (client-side mirror of backend renderer)
// ══════════════════════════════════════════════

function generatePreviewHtml(data: ResumeData): string {
  const {
    fullName,
    email,
    phone,
  ...
  // ── Technical Skills ─────────────────────────
  const skillGroups: { label: string; key: keyof typeof technicalSkills }[] = [
  ...
  const skillsHTML = skillGroups
    ...
    .map(
      (g) =>
        `<div class="skill-row"><b>${g.label}:</b> ${(technicalSkills[g.key] ?? []).join(', ')}</div>`
    )
    .join('\n    ');
...
<div class="page">
  ...
  <div class="edu-block">
    <div class="edu-header">
      ...
      <span class="edu-year">${education.duration?.start ?? ''} – ${education.duration?.end ?? ''}</span>
    </div>
    <div class="edu-school">${education.institution ?? ''}</div>
  </div>` : ''}
