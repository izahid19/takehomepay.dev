...
import { 
  BarChart3, Sparkles, Zap, Cpu, Coins, 
  AlertCircle, ArrowLeft, ChevronRight 
} from 'lucide-react';
import { cn } from '@/lib/utils';

export type AnalysisModel = 'pitchdown-pro' | 'pitchdown-fast' | 'pitchdown-premium-lite';

const MODEL_CREDIT_COST: Record<AnalysisModel, number> = {
  'pitchdown-fast':         1,
  'pitchdown-pro':          2,
...
const MODEL_OPTIONS: {
  ...
  },
  {
    ...
    name: 'PitchDown Fast',
    label: 'Standard V3',
    description: 'Powerful and balanced. A high-performance model offering high accuracy analysis with significantly faster response times.',
    badge: 'Balanced',
  ...
  },
  {
    ...
    label: 'Fast & Smart',
    description: "Our most agile intelligent agent. Blazing-quick resume analysis with high accuracy — great for rapid feedback.",
    badge: 'Fastest',
...
export function ModelSelectionView({
  ...
  const [selected, setSelected] = useState<AnalysisModel>('pitchdown-pro');

  // Filter options based on user plan
  const filteredOptions = MODEL_OPTIONS.filter(opt => {
    if (opt.id === 'pitchdown-premium-lite') {
  ...
  return (
    <div className="relative min-h-[90vh] flex flex-col items-center justify-center py-20 px-6 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-sky-500/5 blur-[120px] rounded-full animate-pulse" />
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16 space-y-4">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] mb-2"
          >
          ...
          >
            Choose Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-blue-400 to-violet-400">AI Analyst</span>
          </motion.h1>
          ...
          >
            Select the AI model to power your resume analysis report.
          </motion.p>
        </div>
        ...
        <div className={cn(
          ...
          filteredOptions.length > 2 ? "lg:grid-cols-3" : "lg:grid-cols-2 max-w-2xl mx-auto"
        )}>
        ...
        <div className="flex flex-col items-center gap-8 relative z-10">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-r from-blue-400/0 via-white/20 to-blue-400/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
